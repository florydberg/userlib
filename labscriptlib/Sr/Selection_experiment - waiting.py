...
for i in range(0,GLOBALS['n_loop']):
    ...
    ## REORDERING BUFFER TIME
    t += 300*msec
    ########################

    if GLOBALS['post_reordering']:
        t+=GLOBALS['holdTime_between_images']
        print('ciao')
        if GLOBALS['release_recapture']:
            print('release them')
        ...
        # t+=Orca_Camera.expose(t-2*Orca_Labscript_delay,'second-shot', trigger_duration=10, saving=True)+orca_trigger_delay+Orca_Labscript_delay
        t+=Orca_Camera.expose(t+5*msec-orca_trigger_delay-Orca_Labscript_delay-GLOBALS['FluoImaging_duration']+20*ms ,'post-reordering', trigger_duration=10, saving=True)+orca_trigger_delay+Orca_Labscript_delay #+5 for sync with fluo
