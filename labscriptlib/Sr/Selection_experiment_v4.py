...
for i in range(0,GLOBALS['n_loop']):
    ...
    if sel_mot_red:
        ...
        if sel_mot_red_sf:
            ...
            if False:
                ...
                n_steps=5
                frq_i=GLOBALS['Red_MOT_Frq']-GLOBALS['Red_MOT_Frq_detuning'] #72.5e6#center of BroadBand Red MOT comb (MHz)
                frq_f=GLOBALS['Red_MOT_Frq']
                pow_i=GLOBALS['Red_MOT_initPow']
                pow_f=GLOBALS['Red_MOT_Pow']
                MOT_Red3D_singleFrq_TTL(t, True)
                for i in range(n_steps):
                    NEW_TABLE_LINE('RedMOT', t-5*usec, (frq_i + (frq_f-frq_i)*i/n_steps)/1e6, pow_i + (pow_f-pow_i)*i/n_steps) #GLOBALS['Red_MOT_Pow']+(n_steps-i)/n_steps*GLOBALS['Red_MOT_Pow']*0.15
                    t+=GLOBALS['MOT_RED_SF_duration']/n_steps

            t+=dt    

            # t+=GLOBALS['MOT_RED_SF_duration']/2
    ...
    if sel_fluo_image:
        ...
        if sel_camera_fluo=='andor': 
            ...
            if co:
                Basler_Camera_extra_trigger.go_high(t-GLOBALS['diagnostic_delay'])
