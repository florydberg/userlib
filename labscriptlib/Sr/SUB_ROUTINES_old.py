...
if True: #Envelope of ttl and analog
    ...
    def TABLE_MODE_ON(channel_name, tt): #gate
        trigger_name = channel_name+'_trigger'
        channel_trigger = globals().get(trigger_name)
        print(str(channel_name ))
        channel_trigger.go_high(tt-5*dt)
        channel_trigger.go_low(tt)


    def TABLE_MODE_OFF(channel_name, tt): #gate
        channel_name = channel_name
        channel = globals().get(channel_name)
        channel.DDS.setamp(tt, amplitude)
        channel.DDS.setfreq(tt, frequency)
    ...
    def COILScomp_SwitchON_TTL(tt, control=True):
        if control:
            coilsMosfet.go_high(t)
        else:
            coilsMosfet.go_low(t)
