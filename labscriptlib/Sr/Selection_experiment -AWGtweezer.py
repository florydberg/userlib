...
if True: ## Selects ##
    ...
    if GLOBALS['Orca_ROI'] == 'full':
        ...
        Orca_Camera.camera_attributes['SUBARRAY HPOS'] = 550 * 4
        ...
        Orca_Camera.camera_attributes['SUBARRAY HSIZE'] = 240
        Orca_Camera.camera_attributes['SUBARRAY VSIZE'] = 240
        Orca_Camera.camera_attributes['SUBARRAY HPOS'] = 650 * 4
        Orca_Camera.camera_attributes['SUBARRAY VPOS'] = 225 * 4
...
for i in range(0,GLOBALS['n_loop']):
    ...
    if sel_mot_red:
        ...
        if sel_mot_red_sf:
            ...
            fluo_delay = 20*ms #GLOBALS['fluo_delay']
        ...
        if sel_tweezer:
            fluo_delay=92*ms - GLOBALS['TweezerLoading_duration'] #found experimantally to when see tweezers start
            ##### Tweezers loading #################
            ...
            tweezer_duration=standingTweezer(t, 'all', amplitude=100, duration = GLOBALS['TweezerLoading_duration']/1e3)*1e4 #this is due to the fact that for now awg doesn't take samples longere than 20us-> Andre put 1e3 loop os the same sample
            awg_trigger.go_high(t+tweezer_duration) # Trigger the AWG to start the sequence
            awg_trigger.go_low(t +tweezer_duration + 2*usec) # Trigger the AWG to start the sequence
            t+=GLOBALS['TweezerLoading_duration']
    ...
    if sel_fluo_image:
        if not sel_abs_image and i == 0: # if no absorption imaging, we can use the same beam for fluorescence, otherwise we can change the frequency and power for fluorescence 
            ImagingBeam.DDS.setfreq(dt, GLOBALS['ImagingFluo_Frq']/1e6*1e3)
        ...
        if 'basler_extra' in sel_camera_fluo:
            ...
            if cb_extra:
                ...
                # Basler_Camera_extra.expose(t-100*usec+5*usec,'FluoExtra', frametype='tiff')
            else:
        ...
        if 'basler_abs' in sel_camera_fluo:
            t+=dt
            Basler_Camera_abs.expose(t-100*usec+5*usec,'FluoAbs', frametype='tiff')
            # camera_readout += exposure_duration
    ...
    if GLOBALS['second_shot']:
        ...
        Sisyphus_AOM_TTL(t+GLOBALS['FluoImaging_duration']+5*dt,False)
        while tt-t < GLOBALS['FluoImaging_duration']/10:
            delta_cooling=GLOBALS['FluoImgCooling_Dt']
            tt+=delta_cooling
            BlueImaging_AOM_TTL(tt+dt,True)
            delta_imaging=GLOBALS['FluoImgPulse_Dt']
    ...
    Shutter_ImagingBlue.go_low(t+2*dt)
    t+=20*ms
