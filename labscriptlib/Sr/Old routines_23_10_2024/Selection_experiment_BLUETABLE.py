...
for i in range(0,GLOBALS['n_loop']):
    ...
    if sel_mot_red:
        ...
        ##########
        t+=300*usec #switching time between high and low voltage
        COILSmain_SwitchON_TTL(t, True)
        t+=dt
        COILSmain_Voltage(t, 0.19)
        t+=dt 
        COILSmain_Current(t, 0.5)#GLOBALS['coils_current_ctrl_red'])          #Turn on the COILSmain at 10 A, roughly 5 G/cm   #Using Vctrl=0.19 and Ictrl=0.5 we minimize the settling time and damp any oscillation
        ###########
    ...
    if sel_fluo_image:
        ...
        if GLOBALS['quantization_ax']:set_CompCoils_QuantizationAxis(t, "ON", GLOBALS['QuantizAxis_ramp_duration'], v_step_size=0.01)
        ...
        if GLOBALS['quantization_ax']:set_CompCoils_QuantizationAxis(t, "OFF", GLOBALS['TOF'])
