...
if awg: # === AWG Tweezer Embedding ===
    ...
    def moving_samples(schedule, dtt, duration, cutting, pos_to_freq_func, move_type, direction, starting_point, linear_ramp , oblique_mode = False,):
        samples = []
        last_time = schedule[0]['t'] * 1e-6
        sequence_offset = schedule[0]['x']
        last_amp = Temperature_to_RF(schedule[0]['A'])
    ...
    def standing_wave_LUT(move_type, frequencies, num_samples, duration, print_crest_factor=True):
        fs = num_samples / duration
        freq_bins = np.fft.rfftfreq(num_samples, d=1/fs)
        inv_gauss = LUT_Tweezer.GaussCorrection(move_type, freq_bins)
        spectrum = np.zeros(len(freq_bins), dtype=np.complex128)
        for f in frequencies:
            ...
            LUT_corr = LUT_Tweezer.get_amp_correction(f, axis=move_type) if GLOBALS['LUT_corr'] else 1
            spectrum[idx] = Gauss_corr *LUT_corr
            print(f"Frequency {f:.2f} Hz: Gauss correction = {Gauss_corr:.3f}, LUT correction = {LUT_corr:.3f}")
            # print(inv_gauss[idx])
        signal = np.fft.irfft(spectrum, n=num_samples) 
        peak = np.max(np.abs(signal))
        if peak:
    ...
    def standingTweezer(tt, Points, amplitude, duration):
        ...
        if isinstance(Points, str) and Points == "all":
            ...
            print(f"Generating standing wave for all points: {Points}")
            name='all'+str(len(Points))
        elif isinstance(Points, int):
            Points = [str(f'P{str(Points)}')]
            name=str(Points)
        ...
        print(f"Generating standing wave for points {Points} with frequencies:")
        for p, fx, fy in zip(Points, fxs, fys):
            print(f"  {p}: fx = {fx:.2f} Hz, fy = {fy:.2f} Hz")
        # fxs = [95e6,100e6,105e6]
        # fys = [95e6,100e6,105e6]
        ...
        # sample_Hor = standing_wave(fxs, amplitude, [0], norm_num_samples, norm_sample_duration)
        # sample_Ver = standing_wave(fys, amplitude, [0], norm_num_samples, norm_sample_duration)

        # Pass signals (single composite wave per axis)
        Horizontal.pass_sample(tt, sample_Hor, f'standing_{name}')
        Vertical.pass_sample(tt, sample_Ver, f'standing_{name}')
    ...
    def connectingTweezerLines(tt, Points, amplitude, linear_ramp=False):
        """
        Generate a composite shifting wave signal for a line of tweezers,
        moving its orthogonal position along.
        ...
        if move_type in ['horizontal', 'vertical']:
            ...
            samples, cut_len = moving_samples(schedule, dtt, duration, cutting, pos_to_freq_func, move_type, direction, offset, linear_ramp)
            shifting_axis = Horizontal if move_type == 'horizontal' else Vertical
            shifting_axis.pass_sample(tt, samples, step_label)
...
if True: # === MOTs ===
    ...
    ##### RED MOT SECTION
        ...
        # tt+=dt
        # NEW_TABLE_LINE('RedMOT', tt, GLOBALS['Red_MOT_Frq_fin']/1e6, GLOBALS['Red_MOT_Pow'])
