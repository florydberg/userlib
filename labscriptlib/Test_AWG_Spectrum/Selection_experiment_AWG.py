...
if True: ## Selects ##
    ...
    if co:
        ...
        if GLOBALS['Orca_ROI']=='full':
            ...
            Orca_Camera.camera_attributes['SUBARRAY HPOS']=577*4
            Orca_Camera.camera_attributes['SUBARRAY VPOS']=198*4
...
for i in range(0,GLOBALS['n_loop']):
    ...
    if sel_mot_red:
        ...
        if sel_tweezer:
            ...
            # NEW_TABLE_LINE('Sisyphus', t, GLOBALS['SisyphusImg_Frq']/1e6, GLOBALS['Sisyphus_Pow'])
    ...
    if sel_fluo_image:
        ...
        if sel_imaging_beam=="abs":
            if sel_tweezer:
                ...
                tt=t
                if not GLOBALS['FluoCoolingPulsing']:
                    NEW_TABLE_LINE('Sisyphus', t, GLOBALS['SisyphusImg_Frq']/1e6, GLOBALS['Sisyphus_Pow'])
                ...
                while tt-t < GLOBALS['FluoImaging_duration']:
                    ...
                    if GLOBALS['FluoCoolingPulsing']:
                        NEW_TABLE_LINE('Sisyphus', tt, GLOBALS['SisyphusImg_Frq']/1e6, GLOBALS['Sisyphus_Pow'])
                        tt+=delta_cooling
...
Vertical.generate_multiple_tones(t,sample_duration=5e-6,frequencies=[(i+1)*1e6], label='vertical'+str(i)) # duration in s, frq in Hz
Horizontal.generate_multiple_tones(t,sample_duration=5e-6,frequencies=[1e7, 1.2e7], label='horizontal'+str(i))
