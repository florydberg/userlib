...
opt_max=120
opt_min=90  #start runmanager with min value in your global setting
opt_step=5
...
TOF=data['TOF']
print('Time of Flight = ', int(TOF.iloc[0]), 'ms' )
...
if True: ##FITTING on GAUSSIAN
    ...
    runmanager.remote.set_globals({'N_Atoms': 110})
    

    plt.show()

old_value=int(data[opt_parameter].iloc[0])
if  old_value in range(opt_min, opt_max):
    new_value=old_value+opt_step
    runmanager.remote.set_globals({opt_parameter: new_value})
    print('old value :', old_value)
    print('new value :', new_value)
    print('optimization still on: new shot on go...')
    runmanager.remote.engage()
else: print('you are done: no more shots for you!')