...
if True:# ROI Selection
    ...
    P0=(520,250)   # Starting point for the atoms ROI
    RX=100
    RY=100
    ...
    p0=(520,320)   # Starting point for the correction area 
    rX=50
    rY=20
...
scan_parameter='Z_Coils_Current_moved'
scan_unit='V'

op_plotting = True #extra images
...
with Run(path).open('r+') as shot:
    ...
    if op_plotting:
        plt.figure()
        plt.imshow(OD, cmap='viridis', vmin=0, vmax=0.3 )
        plt.colorbar()
    ...
    if BlaserAbs_for_Fluo:
        shot_image=shot.get_image('Basler_Camera','Fluo','tiff')
        figure()
