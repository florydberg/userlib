...
def Tweezers_scan(value, title):
    ...
    for jj in range(n_tweezer):
        ...
        x,y,error=data_mean(parameter1, means_ii)
...
para1_unit='dBm'
if duo:
    para2_name='FluoImaging_duration'
    para2_unit='ms'
###################################
...
try: #iniziliation
    ...
    if para1_name=='ImagingFluo_Pow': # conversion dbm to saturation parameter:
        ...
        para1_unit='s'
    elif para2_name=='ImagingFluo_Pow': # conversion dbm to saturation parameter:
    ...
    if duo:
        for ii in range(n_tweezer):
            Tweezers_scan_duo(photons[str(ii+1)],'Fluorescence of Tweezer '+ str(ii+1))
    else:
