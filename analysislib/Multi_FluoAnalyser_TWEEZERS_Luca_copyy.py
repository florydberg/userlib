...
def vector_subtraction(x, x0):
    result = []
    for xi, x0i in zip(x, x0):
        result.append(xi - x0i)
    return result
...
def Tweezers_scan_tot(value, title):
    ...
    y_mean = np.sum(all_y, axis=0)/3  
    errorN_mean = np.sqrt(np.sum(all_errorN**2, axis=0)) / 3
    # errorN_mean = np.abs(vector_subtraction(y, y_mean)) 
    print(errorN_mean) 
    # Subtract offset from x-axis values
    x_adjusted = [2*(xi) for xi in x]
    x_adjusted = [2*(xi-74.85) for xi in x]
    # plt.errorbar(x, y, yerr=errorN, fmt='--o', ecolor='gray',capsize=5)
    plt.errorbar(x_adjusted, y_mean, yerr=errorN_mean, fmt='--o', ecolor='gray',capsize=5)  #
    plt.rcParams.update({'font.size': 20})
...
try: #initialization
    ...
    plt.legend(['Background','Atoms (accumulated)'])  #,
    ...
    if second_shot:
        ...
        data_background=photo_background
        # plt.xlim(-30,100)
        array_photons_2nd = np.concatenate([photons['7'],photons['8'],photons['9'],photons['10'],photons['11']])# 
