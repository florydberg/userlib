...
class SpectrumCard:
    ...
    def set_channel_status(self, active_channels, channel_status = 1): #changed by Andre
        ...
        print('current status: '+str(current_status.value))
    ...
    def change_Multi_IO(self, flag, value):
        """
        Sets a specific flag for the card's I/O operations.

        Parameters:
        - flag: The flag to set (e.g., SPC_F_ENABLEIO).
        - value: The value to set for the flag (1 to enable, 0 to disable).
        """
        if flag == 'X0':
            line = SPCM_X0_MODE
        elif flag == 'X1':
        ...
        if value == 'RUNSTATE':
            cmd = SPCM_XMODE_RUNSTATE
        elif value == 'TRIGOUT':
            cmd = SPCM_XMODE_TRIGOUT
        elif value == 'CONTOUTMARK':
            cmd = SPCM_XMODE_CONTOUTMARK
        elif value == 'DIGOUT':
            cmd = SPCM_XMODE_DIGOUT
        elif value == 'DIGIN':
            cmd = SPCM_XMODE_DIGIN
        elif value == 'ARMSTATE':
            cmd = SPCM_XMODE_ARMSTATE
        else:
            cmd=None


        result = spcm_dwSetParam_i32(self.hCard, line, cmd)
        if result != ERR_OK:
            print(f"Failed to set flag {flag} to {value}.")
            self.handle_error()
        else:
            print(f"Flag {flag} set to {value} successfully.")

        # err = spcm_dwSetParam_i32(self.hCard, SPC_OUT0_MODE, SPC_OUT_MARKER)
        # if err != ERR_OK:
        #     raise Exception('Failed to set X0 to Marker Output mode')
        # else:
        #     print("X0 set to Marker Output mode successfully.") 
    ...
    def card_reset(self):
        ...
        spcm_dwSetParam_i32(self.hCard, SPC_M2CMD , M2CMD_CARD_RESET) # M2CMD_CARD_STOP | M2CMD_CARD_RESET) # (SPC_M2CMD, M2CMD_CARD_RESET | M2CMD_CARD_STOP) 
        self.handle_error()
    ...
    def transfer_sequence_replay_samples(self, segment, samples, ch_number=2):
        """
        ...
        - samples: The array of samples to transfer.
        """
        ch_num=ch_number #change this if using only one channel
        sample_len = len(samples) # number of samples 
        print(f"sample_len={sample_len/ch_num}")
        ...
        if result == ERR_OK:
            nt_sample_len=int(sample_len/ch_num)
            result = spcm_dwSetParam_i32(self.hCard, SPC_SEQMODE_SEGMENTSIZE,  nt_sample_len)
        if result != ERR_OK:
...
def generate_manual_tone(frequency, amplitude, num_samples,  sample_rate = 1.25e9 ):
    """
    Generates a single tone (sine wave) signal with a specified frequency, number of samples, and sample rate.
