...
class AWGOutput(Output):
    ...
    def generate_single_tone(self, t, sample_duration, frequency, label=None):
        ...
        if frequency<fundamental_frequency:
            raise LabscriptError(f"Frequency of '{self.name}' at t={t} is smaller than the resolution ({fundamental_frequency:i}), change frequency or sample size/duration.")

        self.add_instruction(t,(num_samples,frequency,label))

    def generate_multiple_tones(self, t, sample_duration, frequency, amplitudes=None, phases=None, label=None):
        """
        Parameters
        ----------
        t : float
...
class SpectrumAWG(Device):
    ...
    @set_passed_properties(
        property_names={"connection_table_properties": ["device_path","timeout","external_clock_rate","sample_rate","memory_segments", "generation_mode", "trigger_connection"],
                        "device_properties": []                
        }
        )
    def __init__(self, name, device_path, sample_rate, external_clock_rate=None, timeout=5000, generation_mode="single", memory_segments=256, trigger_connection='software', **kwargs):
        ...
        self.triggermode=trigger_connection
