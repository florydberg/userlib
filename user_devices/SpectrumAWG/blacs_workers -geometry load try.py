# Modified by Andre FloRydberg on 07/08/2025
...
class SpectrumAWGWorker(Worker):
    ...
    def geometry_manual(self, values=None, amps=100):
        self.AWG.set_generation_mode(mode='multi')  
        if debugging_mode:
            print(type(values))
            print(self.manual_sample_rate*1e-6)
    ...
    def sequence_thread(self, device_name, h5_file, fresh): #TODO: fix the while logic
        with h5py.File(h5_file,'r') as f:
            ...
            while True:
                try:
                    ...
                    shot_seq_length = len(seq_group['moves'].attrs)
                    print(shot_seq_length)
                    move_labels = group['0'].attrs['labels']

                    if 'move' not in seq_group:
                        print("No 'move' dataset found in reordering sequence.")
