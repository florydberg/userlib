...
def generate_multi_tone(freqs, amps, num_samples, sample_rate):
...
class SpectrumAWGWorker(Worker):
    ...
    def program_manual(self, values=None, amps=100):
        ...
        if isinstance(values, list):
            if debugging_mode:
                print('checking')
            if self.manual_sample_rate <= max(values[0])*1e6*10: #and self.manual_sample_rate>values[1]*10 
        ...
        if self.ch_num==1:
            ...
            if values is None:
                ...
                data0 = generate_multi_tone(freqs_ch0, amps[0], self.num_manual_samples, self.manual_sample_rate)

                data1 = generate_multi_tone( freqs_ch1, amps[1], self.num_manual_samples, self.manual_sample_rate)
