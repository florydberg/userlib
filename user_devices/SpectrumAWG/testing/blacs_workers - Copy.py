...
class SpectrumAWGWorker(Worker):
    def init(self):
        ...
        self.AWG.set_generation_mode(mode='sequence') #or 'single' or 'multi'
    ...
    def transition_to_buffered(self, device_name, h5_file, initial_values, fresh):
        ...
        with h5py.File(h5_file,'r') as f:
            ...
            for ch in self.channels:
                ...
                for index in range(len(group[ch].attrs)): #index of sequences
                    ...
                    if instruction_hash in self.smart_cache:
                        ...
                        if len(instruction) == 2:  # SINGLE TONE
                        ...
                        elif (len(instruction)-1) % 3 == 0:  # MULTI TONE
                    ...
                    if index!=last_index:
                        ...
                        self.AWG.seq_set_sequence_step(index,memory_index,index+1,100,'on_trigger',last_step=False)
                        i_count+=1
                        print(memory_index)
                    else:
                        self.AWG.seq_set_sequence_step(index,memory_index,index,10,'on_trigger',last_step=False)
                        print(memory_index)
                        # If there is a trigger at the stop time, repeat one last sequence 
                        # and then stop (if not card_stop() was called already)
                        self.AWG.seq_set_sequence_step(index+1,memory_index,0,10,'always',last_step=True)
                # break 
