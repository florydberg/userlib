...
class power_check_boxes(QWidget):
    ...
    def _onPID(self, parent, channel, state):
        # executed by QT main thread (tab_base_classes.py Tab::mainloop). must be generator (with yield).
        info = "'%s' PID %s" % (self.name, 'enable' if state else 'disable')
...
def PID_boxes(QWidget):
    # Andre: PID box for each DDS
    labels = ['Proportional', 'Integral', 'Derivative', 'Setpoint']  # labels for check boxes

    def __init__(self, parent, name, channel, Proportional=False, Integral=False, Derivative=False, Setpoint=False, align_horizontal=False):
        ...
        # Capabilities
        self.PID.base_units = {'proportional': ' ', 'integral': ' ', 'derivative': ' '}
        self.PID.base_min = {'proportional': 0, 'integral': 0, 'derivative': -1}
        self.PID.base_max = {'proportional': 100, 'integral': 100, 'derivative': +1}
        self.PID.base_step = {'proportional': 0, 'integral': 0, 'derivative': 0}
        self.PID.base_decimals = {'proportional': 3, 'integral': 3, 'derivative': 1}
        self.PID.num_DDS = MAX_NUM_CHANNELS
        ...
        for i in range(self.num_DDS):  # 4 is the number of DDS outputs on this device
            ...
            for subchnl in ['proportional', 'integral', 'derivative']:
                PID_prop['channel %d' % i][subchnl] = {'base_unit': self.PID.base_units[subchnl],
                                                       'min': self.PID.base_min[subchnl],
                                                       'max': self.PID.base_max[subchnl],
                                                       'step': self.PID.base_step[subchnl],
                                                       'decimals': self.PID.base_decimals[subchnl]
                                                       }
        # Create UI
        self.PID = UiLoader().load(os.path.join(os.path.dirname(os.path.realpath(__file__)),"./MOGLABS_blacs_widget.ui"))
        self.PID.channel=channel

        # self.PID.pushButton_SingleTone.clicked.connect(lambda: self.onPID())
        # self.PID.pushButton_MemoryReplay.clicked.connect(lambda: self.onPID())
        # self.PID.pushButton_refresh.setIcon(QtGui.QIcon(':/qtutils/fugue/arrow-circle-double'))
        # self.PID.pushButton_refresh.clicked.connect(lambda: self.onPID())
        self.PID.pushButton_invert.setIcon(QtGui.QIcon(':/qtutils/fugue/arrow-circle-double'))
        self.PID.pushButton_invert.clicked.connect(lambda: self.invertPID())

        self.auto_place_widgets((f"P.I.D. settings for channel {ch}",{"P.I.D.":self.PID}))

    def invertPID(self, state=True):
        # 'PID' clicked: manually insert event into parent event queue. see tab_base_classes.py @define_state(MODE_MANUAL, True)
        self.parent.event_queue.put(allowed_states=MODE_MANUAL, queue_state_indefinitely=True, delete_stale_states=False,
                                    data=[self._invertPID, [[self.PID.channel, state], {}]])
...
class MOGLabs_QRF_Tab(DeviceTab):
    def initialise_GUI(self):
        ...
        for channel in range(4):
            pb = PID_boxes(parent=self, name=channels[channel], channel=channel_index, align_horizontal=place_below)
            if place_below: layout.addWidget(pb)
            else:           layout.addWidget(pb,1,1)
...
class MOGLabs_QRF_Worker(Worker):
    ...
    def setPID(self, channel, setting, value):
        ...
        info = "'%s' channel %i: PID's %s is set to %f" % (self.device_name, channel, setting, value)
        if self.dev is not None:
            self.dev.cmd('PID, GAIN, %i, %s , %f' % (channel + 1, setting, value))
            print(info)
            return True
        print(info + ' failed!')
        return False
     
    def setpointPID(self, channel, value): #value must be [-1,+1]
        # Andre: PID setpoint: applies a DC offset to anable locking at non-zero setpoint voltage. returns True if ok, False on error.
        info = "'%s' channel %i: PID's setpoint at %f" % (self.device_name, channel, setting, value)
        if self.dev is not None:
            self.dev.cmd('PID, SETPOINT, %i, %f' % (channel + 1, value))
            print(info)
            return True
        print(info + ' failed!')
        return False
    ...
    def invertPID(self, channel, state):
        ...
        if self.dev is not None:
            self.dev.cmd('PID, INVERT, %i, %s' % (channel + 1, cmd))
            print(info)
            return True
        print(info + ' failed!')
        return False   
    ...
    def errorPID(self, channel):
        ...
        return self.dev.cmd('PID, ERROR, %i' % (channel + 1))
