...
def check_worker_args(worker_args):
    ...
    for key,value in worker_args.items():
        ...
        if (key == STR_CONFIG) or (key == STR_CONFIG_MANUAL):
            ...
            if not isinstance(valid, boolean):
        ...
        if self.sock == None: # should never happen
            ...
            if result is None:
                ...
                if (cmd != SERVER_STATUS_RSP) and (cmd != SERVER_STATUS_IRQ_RSP):
                    ...
                    #elif self.board_status & STATUS_WAIT: # wait state = start trigger #old code to be replaced by next line
                    elif (self.board_status & STATUS_WAIT) or self.start_trg: # new code.
                        checks if waits for start trigger
    ...
    def parse_worker_args(self, worker_args):
        ...
        if len(worker_args) > 0:
            ...
            if STR_INPUTS in worker_args and worker_args[STR_INPUTS] is not None:
                ...
                # start_trg = is_in_start(self.ctrl_in) # old code
                self.start_trg = is_in_start(self.ctrl_in) # new code
