...
class DigitalChannels(IntermediateDevice):
    ...
    def add_device(self, device):
        ...
        device.rack, device.address, device.channel = check_device(device, DigitalOut, self.max_channels, self.child_devices, self.shared_address) #was True
        device.type = TYPE_DO
...
def init_connection(device_name, con, config):
    ...
    if sock is None: print("%s: connection %s failed!" % (device_name, con))
        ...
        send_recv_data(sock, SERVER_CLOSE, 0.1, output='clowse')
        sock.close()
    return None
