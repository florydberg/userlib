...
# copy into folder: site-packages/labscript-utils/unitconversions/
# TODO: I could not get this working when in separate file in pythonlib or userlib or when class is within FPGA_device.py file.
#       error: self.create_analog_outputs(ao_prop) in BLACS tab calls for each channel in ao_prop
#       get_unit_conversion_class(fullname) in unitconversions/__init__.py which returns None for fullname='unit_conversion'
#       this is although import generic_conversion class works without problems!?
#       the root problem seems that globals() called from FPGA_device.py is different than called from get_unit_conversion in unitconversions/__init__.py.
#       could not find online if this behavior is normal or not? in any case its strange that a global is not global but context dependent.
#       I tried to manually add generic_conversion to globals, globals()['generic_conversion']=generic_conversion, but without success.
#       giving unit_conversion_class='path.to.some.module.ClassName' gives error in labscript.py Output(Device)::__init__ that a string is not callable.
#       giving unit_conversion_class=path.to.some.module.ClassName with path starting from folder labscriptlib, pythonlib or user_devices
#       gives again fullname='unit_conversion' with the path removed. note also that the fullname is always a string and not the class object.

# only additional dependency is on numerical python
import numpy as np

# TODO: when this file is part of labscript source code I have problems with importing!
# - importing from FPGA_device does not work (maybe could give a full path and import with importlib)
# - importing LabscriptError does not work. if I write 'from labscript.labscript import *' importing does not complain but LabscriptError is NOT imported although is there?
# - I managed to import UnitConversion class but the same code does not work when this file is not in labscript source.
# - running script for newton_test does not work. get 'double import' error!? comment line with UnitConversionBase and delete UnitConversion inherited class, then it works.

# True = this file is part of labscript source (site-packages/labscript-utils/unitconversions/) TODO: remove this option since works now more or less.
# False = this file is in labscriptlib/<experiment> or user_devices/ or pythonlib folder (default)
in_labscript_source = False

if in_labscript_source:
    from .UnitConversionBase import *
    #from labscript.labscript import LabscriptError # does not work!?
    class LabscriptError(Exception):
        pass
    #from user_devices.FPGA_device import default_ao_props  # does not work!?
    default_ao_props = {'base_unit':'V', 'min':-10.0, 'max':10.0, 'step':1.0, 'decimals':4} # cannot import from FPGA_device!?
else:
    from labscript import UnitConversion, LabscriptError
    from user_devices.FPGA_device import default_ao_props # does not work!? TODO: maybe works now with new labscript version!?
    #default_ao_props = {'base_unit': 'V', 'min': -10.0, 'max': 10.0, 'step': 1.0, 'decimals': 4}  # cannot import from FPGA_device
